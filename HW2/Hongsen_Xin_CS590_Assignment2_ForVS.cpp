#include <fstream> // For VS complier, should add key word "typename" before every "SList<T>::Iterator". Then the code can be compiled.
#include <iostream>
#include <vector>
#include <assert.h>
...
template<class T>
void SList<T>::AddAfter(typename SList<T>::Iterator i, const T &newItem){ //For VS complier, should add key word"typename" before  SList<T>::Iterator
    assert(i.position != 0);
...
template <class T>
void SList<T>::RemoveAfter(typename SList<T>::Iterator i){ //For VS complier, should add key word"typename" before  SList<T>::Iterator
...
SList<int>::Iterator i2;
	...
	system("pause"); // For windows pause to show results
    return 0;
}
